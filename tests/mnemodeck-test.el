...
(ert-deftest mnemodeck-test-edit-delete-marked-branch-runs-batch-delete ()
  (let ((deleted '())
    ...
    (cl-letf (((symbol-function 'mnemodeck-edit--marked-words)
               (lambda () '("a" "b")))
              ((symbol-function 'mnemodeck-edit--nearest-surviving-word)
              ...
              ((symbol-function 'mnemodeck-edit--line-of-word)
               (lambda (word)
                 (when (string= word "next-word")
                   5)))
              ((symbol-function 'mnemodeck-edit--restore-position)
...
(ert-deftest mnemodeck-test-edit-nearest-surviving-word-prefers-forward-on-tie ()
  (with-temp-buffer
    (insert "a\nb\nc\nd\n")
    (goto-char (point-min))
    (forward-line 2)
    (cl-letf (((symbol-function 'tabulated-list-get-id)
               (lambda ()
                 (pcase (line-number-at-pos)
                   ...
                   (4 "d")
                   (_ nil)))))
      (should (string= (mnemodeck-edit--nearest-surviving-word '("c")) "d"))
      (should (string= (mnemodeck-edit--nearest-surviving-word '("b" "c")) "d")))))
...
(ert-deftest mnemodeck-test-edit-archive-marked-in-archived-filter-unarchives-and-restores ()
  (let ((mnemodeck-edit--filter 'archived)
        (unarchived '())
        (archived nil)
        (ensured nil)
    ...
    (cl-letf (((symbol-function 'mnemodeck-edit--marked-words)
              ...
              ((symbol-function 'mnemodeck-edit--ensure-not-current)
               (lambda (words) (setq ensured words)))
              ((symbol-function 'yes-or-no-p) (lambda (&rest _) t))
              ((symbol-function 'mnemodeck-unarchive-card)
               (lambda (word) (push word unarchived)))
              ((symbol-function 'mnemodeck-archive-card)
...
(ert-deftest mnemodeck-test-edit-archive-marked-in-all-filter-archives-and-restores ()
  (let ((mnemodeck-edit--filter 'all)
        (unarchived nil)
        (archived '())
        (ensured nil)
        (cleared nil)
...
(ert-deftest mnemodeck-test-edit-delete-marked-without-survivor-skips-restore ()
  (let ((restored nil))
    (cl-letf (((symbol-function 'mnemodeck-edit--marked-words)
               (lambda () '("a" "b")))
              ((symbol-function 'mnemodeck-edit--nearest-surviving-word)
...
(ert-deftest mnemodeck-test-edit-delete-marked-cancelled-skips-side-effects ()
  (let ((deleted nil)
        (cleared nil)
        (refreshed nil)
        (restored nil)
...
(ert-deftest mnemodeck-test-edit-archive-marked-cancelled-skips-side-effects ()
  (let ((mnemodeck-edit--filter 'all)
        (archived nil)
        (unarchived nil)
        (cleared nil)
